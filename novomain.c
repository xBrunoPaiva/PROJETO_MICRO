...
void loop() {
  ...
  if (Serial1.available()) {
    ...
    if (recebido.startsWith("{{")) {
      ...
      if (tabuleiro[4][4] != 11) {
        while (idx < recebido.length() && linha < 7) {
          int espaco = recebido.indexOf(',', idx);
          if (espaco == -1) espaco = recebido.length();
          String peca = recebido.substring(idx, espaco);
          peca.trim();
        ...
        while (idx < recebido.length() && linha < 7) {
          ...
          peca.trim();
          int valor = peca.toInt();
          tabuleiroNovo[linha][coluna++] = valor;
          if (coluna == 7) {
            coluna = 0;
            linha++;
      ...
      for (int i = 0; i < 7; i++) {
        for (int j = 0; j < 7; j++) {
          Serial.print(tabuleiroNovo[i][j]);
          Serial.print(" ");
        }
        Serial.println();
      }
  ...
  if (comparaMatrizes()) {
    ...
    int peca = tabuleiro[linzero][colzero];
...
bool comparaMatrizes() {
  for (int i = 0; i < LIN; i++) {
    for (int j = 0; j < COL; j++) {
      if (tabuleiro[i][j] != tabuleiroNovo[i][j]) {
        linzero = i;
        colzero = j;
        return true;
      }
