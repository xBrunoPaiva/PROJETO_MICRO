...
void loop() {
  ...
  if (Serial1.available()) {
    ...
    if (recebido.startsWith("{{")) {
      Serial.println("chegou a matriz");
      recebido.replace("{", "");
      recebido.replace("}", "");
      geraMatriz();
    ...
    // 3) PUT Xn → coloca no destino
      ...
      if (!ok) {
        ...
        int msgH = 2 * 16;                    // duas linhas de recebido em size=2 (~16px de altura cada)
